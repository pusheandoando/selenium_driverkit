# selenium_driverkit/platforms/linux/firefox.py
import os
import re
import tarfile
import requests
...
def _get_browser_version():
    ...
    raise RuntimeError(
        "[selenium_dk] could not detect Firefox version, make sure is installed."
    )
...
def get_driver_firefox(download_path: str, auto_update: bool = True):
    drivers_root = os.path.join(download_path, "Drivers", "Firefox", "Linux")
    os.makedirs(drivers_root, exist_ok=True)

    binary_path = os.path.join(drivers_root, "geckodriver")

    print('\n')
    print("[ Firefox WebDriver (Linux) ]")
    ...
    try:
        ...
        print(f" - Architecture: {os.uname().machine} ({arch_suffix})")
    except RuntimeError as e:
        print(f" [!!] {e}")
        return None
